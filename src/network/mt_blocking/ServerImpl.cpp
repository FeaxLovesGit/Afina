...
//            tv.tv_sec = 5; // TODO: make it configurable
        ...
        if (_threads.size() < max_thread_count) {
            std::lock_guard<std::mutex> lock(mtx);
            _threads[client_socket] = std::thread(&ServerImpl::_process_connection, this, client_socket);
        }
        else {
            static const std::string msg = "Try access LATER";
...
void ServerImpl::_process_connection(int client_socket) {
    ...
    {
        std::lock_guard<std::mutex> lock(mtx);
        std::thread &thr = _threads[client_socket];
        thr.detach();
        _threads.erase(client_socket);
